...
import useUploadFiles from "../../../Other/useUploadFiles"
  ...
  const loadFiles = async() => {
    if(dataTransfer.files.length > 0) {
      try {
        ...
        Promise.allSettled(promises).then(async(data) => setFiles((await request('GET', process.env.REACT_APP_SERVER_HOST + `/api/storage/${storage}/`)).files))
  ...
  return (
    <div className="files-upload">
      <div className="file-container" onClick={handleFileClick}>
        ...
        <span className="overlap">Загрузить файлы</span>
      </div>
                ...
                {Array.from(dataTransfer.files).map((file, i) => {
                  return (
                    <tr key={i}>
                      ...
                      <td className='td-pointer' name={`comment_${file.name}_${file.size}`} onClick={changeField}>
                        {statusField.field_edit === `comment_${file.name}_${file.size}` && statusField.status_edit && !load ? 
                        <FieldInput statusField={statusField} setStatusField={setStatusField} object={comments} setObject={setComments} /> : comments[`comment_${file.name}_${file.size}`]}
                      </td> 
                      {!load ? <td className="file-load-td-pointer"><span className="file-delete" onClick={() => fileDelete(i, file.name, file.size)}>&#10060;</span></td> : ''}
                    </tr>)
                })}
            </tbody>
          </table>
          <div className="files-save">
          {!load ? <input type='button' className="files-btn-upload" value="Отправить" onClick={loadFiles} /> : <ProgressBar progress={progress} />}
          </div>
        </div></>
      : ''}
    </div>
