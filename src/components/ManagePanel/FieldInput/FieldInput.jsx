...
function FieldInput({ statusField, setStatusField, object, setObject }) {
  ...
  const handleSave = (e) => {
    ...
    let pattern = /^((?!\.)[\w\-_.]*[^.])(@\w+)(\.\w+(\.\w+)?[^.\W])$/gi   // Тестирование e-mail
     switch(name) {
      case 'first_name':
        pattern = /^(?=.{2,20}$)[A-Za-zА-Яа-я]+$/g          // Тестирование имени
        if(pattern.test(state.input)) {
          setObject(() => ({...object, [name]: state.input}))
        }        
        break
      case 'last_name':
        pattern = /^(?=.{2,20}$)[A-Za-zА-Яа-я]+$/g          // Тестирование фамилии
      ...
      case 'password':
        ...
        break
      case 'name_origin':
        pattern = /^[0-9a-zA-Zа-яА-Я_\-. ]+$/g          // Тестирование имени файла Windows
