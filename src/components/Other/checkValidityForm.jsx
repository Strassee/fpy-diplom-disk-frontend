function checkValidityForm(form, errors, stateForm, setStateForm, setTooltip, patternsInput = false) { 
  const stateF = {};
  for (let name in stateForm) {
    stateF[name] = String(stateForm[name]).trim();
    // setStateForm((prevForm) => ({...prevForm, [name]: String(stateForm[name]).trim()}));
  }
  setStateForm(stateF);
